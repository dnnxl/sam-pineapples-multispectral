...
from data import get_foreground, Transform_To_Models, get_background, Multispectral_Transforms
...
class MahalanobisFilter:
    ...
    def __init__(self,
                ...
                dim_red=None, 
                bands_to_apply=None,
                n_components=10, feature_extractor=None, multispectral=False,
        ...
        #else:
        #    trans_norm = Multispectral_Transforms(bands_to_apply=bands_to_apply.split(","))
        
        if use_dofa_model:
            #trans_norm = Transform_To_Models()
            trans_norm = Multispectral_Transforms(size=(224, 224), bands_to_apply=bands_to_apply.split(","))
        elif self.feature_extractor.is_transformer:
            #trans_norm = Transform_To_Models(
            #        size=feature_extractor.input_size, 
            #        force_resize=True, keep_aspect_ratio=False
            #    )
            trans_norm = Multispectral_Transforms(size=self.feature_extractor.input_size, bands_to_apply=bands_to_apply.split(","))
        else:
            #trans_norm = Transform_To_Models(
    ...
    def run_filter(self,
        ...
        num_classes:float=0, 
        mahalanobis_method="regularization", beta=1, seed=10, lambda_mahalanobis=-1.0, multispectral=False):
        ...
        for (batch_num, batch) in tqdm(
            ...
            for idx in list(range(batch[1]['img_idx'].numel())):
                ...
                imgs_f, labels_f = get_foreground(
                    ...
                    self.use_sam_embeddings, multispectral=multispectral
                )
                labeled_imgs += imgs_f
                ...
                if get_background_samples:
                    ...
                    imgs_b, labels_b = get_background(
                        ...
                        num_classes, self.use_sam_embeddings, multispectral=multispectral)
        ...
        print("--------------------------------------------------------")
        print("All labeled features: ", all_labeled_features.shape)
        ...
        self.evaluate(unlabeled_loader, dir_filtered_root, "bbox_results", multispectral=multispectral)
        self.evaluate(validation_loader, dir_filtered_root, "bbox_results_val", multispectral=multispectral)
        

    def evaluate(self, dataloader, dir_filtered_root, result_name, multispectral=False):
        ...
        for (batch_num, batch) in tqdm(
            ...
            for idx in tqdm(list(range(batch[1]['img_idx'].numel())), desc="Iterate images"):
                ...
                imgs_s, box_coords, scores = self.sam_model.get_unlabeled_samples(
                    batch, idx, self.trans_norm, self.use_sam_embeddings, multispectral=multispectral
                )
    ...
    def get_all_features(self, images):
        ...
        if self.use_sam_embeddings:
            ...
            with torch.no_grad():
                for img in images:
                    img = img.unsqueeze(0) 
                    t_temp = self.feature_extractor(img.to(self.device))#.unsqueeze(dim=0).to(self.device))
                    features.append(t_temp.squeeze().cpu())
        else:
            with torch.no_grad():
                for img in images:
                    t_temp = self.feature_extractor.forward_features(img.to(self.device))#.unsqueeze(dim=0).to(self.device))
                    print(t_temp.shape)
                    features.append(t_temp.squeeze().cpu())
        return features